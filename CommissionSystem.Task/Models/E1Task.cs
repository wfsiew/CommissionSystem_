                    ...
                    for (int j = 0; j < l.Count; j++)
                    {
                        ...
                        foreach (KeyValuePair<int, Customer> d in customerDic)
                        {
                            ...
                            if (a.IsInternalVoice)
                            {
                                ...
                                stdrate = cr.STD > E1Internal.MaxRate ? E1Internal.MaxRate : cr.STD;
                                mobrate = cr.MOB > E1Internal.MaxRate ? E1Internal.MaxRate : cr.MOB;

                                if (!sf.IDDInternalSetting.ContainsKey(iddrate))
                                    iddrate = 0;

                                if (!sf.E1InternalSetting.ContainsKey(stdrate))
                                    stdrate = 0;

                                if (!sf.E1InternalSetting.ContainsKey(mobrate))
                                    mobrate = 0;

                                v.CommissionRateIDD = iddrate > 0 ? sf.IDDInternalSetting[iddrate].Commission : 0;
                                v.CommissionRateSTD = stdrate > 0 ? sf.E1InternalSetting[stdrate].Commission : 0;
                                v.CommissionRateMOB = mobrate > 0 ? sf.E1InternalSetting[mobrate].Commission : 0;
                                ...
                                v.CommissionMOB = mobrate > 0 ? sf.E1InternalSetting[mobrate].GetDirectCommission(v.CallChargeMOB) : 0;
                                ...
                                for (int n = 1; n < 3; n++)
                                {
                                    ...
                                    for (int z = 0; z < blist.Count; z++)
                                    {
                                        ...
                                        bv.CommissionSTD = stdrate > 0 ? sf.E1InternalSetting[stdrate].GetCommission(v.CallChargeSTD, n) : 0;
                                        bv.CommissionMOB = mobrate > 0 ? sf.E1InternalSetting[mobrate].GetCommission(v.CallChargeMOB, n) : 0;
                            ...
                            else
                            {
                                ...
                                stdrate = cr.STD > E1External.MaxRate ? E1External.MaxRate : cr.STD;
                                mobrate = cr.MOB > E1External.MaxRate ? E1External.MaxRate : cr.MOB;
                                ...
                                v.CommissionRateSTD = stdrate > 0 ? sf.E1ExternalSetting[stdrate].Commission : 0;
                                v.CommissionRateMOB = mobrate > 0 ? sf.E1ExternalSetting[mobrate].Commission : 0;
                                ...
                                v.CommissionMOB = mobrate > 0 ? sf.E1ExternalSetting[mobrate].GetDirectCommission(v.CallChargeMOB) : 0;
                                ...
                                for (int n = 1; n < 4; n++)
                                {
                                    ...
                                    for (int z = 0; z < blist.Count; z++)
                                    {
                                        ...
                                        bv.CommissionSTD = stdrate > 0 ? sf.E1ExternalSetting[stdrate].GetCommission(v.CallChargeSTD, n) : 0;
                                        bv.CommissionMOB = mobrate > 0 ? sf.E1ExternalSetting[mobrate].GetCommission(v.CallChargeMOB, n) : 0;
        ...
        private Dictionary<int, Customer> GetCustomers()
        {
            ...
            try
            {
                ...
                sb.Append("select custid, name, rateplanid, billingday, status from customer where agentid = @agentid and serviceid = 13 and ")
                    .Append("name like @keyword and custid in (")
                    .Append("select distinct custid from customersettlement where paymenttype <> 2 and ")
                    .Append("realdate >= @datefrom and realdate < @dateto)");
                string q = sb.ToString();
