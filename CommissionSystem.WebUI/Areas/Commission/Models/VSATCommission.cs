...
{
    public class VSATCommission : IDisposable
    {
                    ...
                    for (int j = 0; j < l.Count; j++)
                    {
                        ...
                        foreach (KeyValuePair<int, Customer> d in customerDic)
                        {
                            ...
                            if (a.IsInternalData)
                            {
                                ...
                                v.Commission = sf.VSATInternalSetting.GetDirectCommission(v.SettlementAmount);
                                ...
                                for (int n = 1; n < 3; n++)
                                {
                                    ...
                                    for (int z = 0; z < blist.Count; z++)
                                    {
                                        ...
                                        bv.Commission = sf.VSATInternalSetting.GetCommission(v.SettlementAmount, n);
                                        bv.CommissionRate = sf.VSATInternalSetting.GetCommissionRate(n);
                            ...
                            {
                                v.CommissionRate = sf.VSATExternalSetting.Commission;
                                v.SettlementAmount += amount;

                                av[agentid].TotalSettlement += v.SettlementAmount;

                                v.Commission = sf.VSATExternalSetting.GetDirectCommission(v.SettlementAmount);
                                av[agentid].TotalCommission += v.Commission;
                                ...
                                for (int n = 1; n < 4; n++)
                                {
                                    ...
                                    for (int z = 0; z < blist.Count; z++)
                                    {
                                        ...
                                        CommissionView bv = new CommissionView();
                                        bv.Customer = customer;
                                        bv.Commission = sf.VSATExternalSetting.GetCommission(v.SettlementAmount, n);
                                        bv.CommissionRate = sf.VSATExternalSetting.GetCommissionRate(n);
                                        bv.SettlementAmount += v.SettlementAmount;
                                        cv[bagentid].Add(bv);
