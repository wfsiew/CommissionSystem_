function CorporateInternetPremiumCtrl($scope, $http, $modal) {
    $scope.agentID = '';
    $scope.agents = [];
    ...
    $scope.showCommission = function () {
        ...
        var url = route.corporateinternetpremium.commission;
        utils.blockUI();
    ...
    $scope.sendMail = function () {
        ...
        var url = route.corporateinternetpremium.mail;
        utils.blockUI();
        $http.post(url, o).success(function (data) {
            utils.unblockUI();
            if (data.success == 1)
    ...
    $scope.init = function () {
        var url = route.corporateinternetpremium.agents;
